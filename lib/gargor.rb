...
class Gargor
  class << self
    ...
    def prev_count(g = @@prev_generation)
      # fitness > 0 適応している個体
      g.count { |i| i.fitness && i.fitness > 0 }
    end
    ...
    def populate_first_generation
      ...
      unless individuals
        ...
        individuals << base
        individuals << mutate until individuals.length >= opt('population')
      end
    ...
    def total_trials
      opt('population') + (opt('population') -
                           opt('elite')) * (opt('max_generations') - 1)
    end

    def last_trials_at_this_generation
      @@individuals.count { |i| i.fitness.nil? }
    end
